...
export interface ProviderPortalScheduleBlockRow {
  id: string;
  dayOfWeek: number;
  startTime: string;
  endTime: string;
...
export function ProviderPortalAvailabilityTab({ blocks }: { blocks: ProviderPortalScheduleBlockRow[] }) {
  ...
  async function handleDelete(scheduleId: string) {
    ...
    try {
      const res = await fetch(`/api/provider/schedule/${scheduleId}`, { method: "DELETE" });
      if (!res.ok) {
  ...
  const grouped = React.useMemo(() => {
    const map = new Map<number, ProviderPortalScheduleBlockRow[]>();
    ...
    <div className="space-y-4">
      ...
      {grouped.length === 0 ? (
        <EmptyState
          ...
          description="Add your weekly availability blocks so the billing office knows when you're seeing patients."
        />
      ) : (
