import { CalendarClock, ReceiptText, ShieldCheck } from "lucide-react";
import { Kicker } from "@/components/shared/kicker";

const HIGHLIGHTS = [
  {
    icon: CalendarClock,
    title: "See your schedule",
    body: "Every appointment assigned to you, past and upcoming, in one place.",
  },
  {
    icon: ReceiptText,
    title: "Track your claims",
    body: "Check the status and balance of claims billed under your NPI.",
  },
  {
    icon: ShieldCheck,
    title: "Stay credentialed",
    body: "See your license, DEA, and malpractice insurance expirations at a glance.",
  },
];

export function ProviderPortalBrandPanel() {
  return (
    <div className="relative hidden h-full flex-col justify-between overflow-hidden bg-sidebar p-12 text-sidebar-foreground lg:flex">
      <div
        className="pointer-events-none absolute inset-0 opacity-40"
        style={{
          backgroundImage:
            "radial-gradient(circle at 15% 20%, rgba(16,185,129,0.35), transparent 45%), radial-gradient(circle at 85% 80%, rgba(20,184,166,0.25), transparent 40%)",
        }}
      />

      <div className="relative z-10 flex items-center gap-2.5">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary text-primary-foreground font-bold">
          M
        </div>
        <div className="leading-tight">
          <p className="text-sm font-semibold">MedBill</p>
          <p className="text-[11px] uppercase tracking-wider text-sidebar-muted">Provider Portal</p>
        </div>
      </div>

      <div className="relative z-10 max-w-md space-y-6">
        <Kicker>Provider Portal</Kicker>
        <h1 className="text-3xl font-bold leading-tight tracking-tight">
          Your schedule, claims, and credentials, <span className="text-primary-400">in one place.</span>
        </h1>
        <div className="space-y-6 pt-4">
          {HIGHLIGHTS.map((item) => (
            <div key={item.title} className="flex gap-3.5">
              <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-sidebar-accent">
                <item.icon className="h-4.5 w-4.5 text-primary-300" />
              </div>
              <div>
                <p className="text-sm font-medium">{item.title}</p>
                <p className="mt-0.5 text-sm text-sidebar-muted">{item.body}</p>
              </div>
            </div>
          ))}
        </div>
      </div>

      <p className="relative z-10 text-xs text-sidebar-muted">
        © {new Date().getFullYear()} MedBill RCM Suite. All rights reserved.
      </p>
    </div>
  );
}
