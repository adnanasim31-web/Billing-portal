...
export function MobileNav({ organization }: { organization: OrganizationSummary | null }) {
  ...
  return (
    <>
      ...
      {open &&
        createPortal(
          // Rendered via portal, not as a descendant of the navbar - the navbar's `backdrop-blur`
          // makes it a containing block for `position: fixed` descendants, which would otherwise
          // clip this overlay to the navbar's own height instead of the full viewport.
