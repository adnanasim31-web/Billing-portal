...
import { listPatientNotes, addPatientNote } from "@/lib/services/patient-notes-service";
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const notes = await listPatientNotes(id, user.organizationId);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = patientNoteSchema.safeParse(body);
  if (!parsed.success) {
